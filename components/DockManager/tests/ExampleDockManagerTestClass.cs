...
namespace DockManagerExperiment.Tests;

[TestClass]
public partial class ExampleDockManagerTestClass : VisualUITestBase
{
    // If you don't need access to UI objects directly or async code, use this pattern.
    [TestMethod]
    public void SimpleSynchronousExampleTest()
    {
        var assembly = typeof(DockManager).Assembly;
        var type = assembly.GetType(typeof(DockManager).FullName ?? string.Empty);

        Assert.IsNotNull(type, "Could not find DockManager type.");
        Assert.AreEqual(typeof(DockManager), type, "Type of DockManager does not match expected type.");
    }
    ...
    [UIThreadTestMethod]
    public void SimpleUIExamplePageTest(ExampleDockManagerTestPage page)
    {
        // You can use the Toolkit Visual Tree helpers here to find the component by type or name:
        var component = page.FindDescendant<DockManager>();

        Assert.IsNotNull(component);

        var componentByName = page.FindDescendant("DockManagerControl");
    ...
    [UIThreadTestMethod]
    public async Task SimpleAsyncUIExamplePageTest(ExampleDockManagerTestPage page)
    {
